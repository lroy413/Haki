...
export async function readAllTables(db: Db): Promise<BackupTables> {
  const [
    ...
    db.select().from(headway).orderBy(headway.createdAt),
  ...
  return {
    ...
    // Keyed by day alone, so it is its table's row with nothing to look up.
    weatherReading: shifts.map((r) => ({
      day: r.day,
      word: r.word,
      note: r.note,
      createdAt: r.createdAt,
