...
import { getRead, recentSleep } from '../db/repo';
...
export function HakiProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    const [todayRead, nights] = await Promise.all([getRead(db, today), recentSleep(db, 7, today)]);
  ...
  const value = useMemo<HakiState>(
    ...
    [read, reserve, cascade, settings.plainMode, settings.setSailAt, refresh],
  );
